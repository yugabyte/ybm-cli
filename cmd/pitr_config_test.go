...
test_ysql_db   YSQL         5                          86400                        QUEUED    654321`))
	...
	var _ = Describe("Describe Cluster PITR Config", func() {
		It("Should successfully describe PITR Configs for the YSQL namespace in the cluster", func() {
			...
			Expect(ysqlErr).NotTo(HaveOccurred())
			ysqlSession.Wait(2)
			Expect(ysqlSession.Out).Should(gbytes.Say(`Namespace      Table Type   Retention Period in Days   Backup Interval in Seconds   State     Earliest Recovery Time in Millis
test_ysql_db   YSQL         5                          86400                        QUEUED    654321`))
			ysqlSession.Kill()
		})
		...
		It("Should successfully describe PITR Configs for the YCQL namespace in the cluster", func() {
			...
			Expect(ycqlErr).NotTo(HaveOccurred())
			ycqlSession.Wait(2)
			Expect(ycqlSession.Out).Should(gbytes.Say(`Namespace      Table Type   Retention Period in Days   Backup Interval in Seconds   State     Earliest Recovery Time in Millis
test_ycql_db   YCQL         6                          86400                        ACTIVE    123456`))
			ycqlSession.Kill()
		})
