...
var _ = Describe("PITR Configs Test", func() {
	...
	var _ = Describe("List Cluster PITR Configs", func() {
		It("Should successfully list PITR Configs for the cluster", func() {
			...
			session.Wait(2)
			Expect(session.Out).Should(gbytes.Say(`Namespace      Table Type   Retention Period in Days   Backup Interval in Seconds   State     Earliest Recovery Time in Millis
test_ycql_db   YCQL         6                          86400                        ACTIVE    123456
test_ysql_db   YSQL         5                          86400                        QUEUED    654321`))
			session.Kill()
		})
	...
	var _ = Describe("Create PITR config", func() {
		It("Should successfully create PITR config", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			session.Wait(2)
			Expect(session.Out).Should(gbytes.Say(`The PITR Configuration for namespace test_ysql_db in cluster stunning-sole is being created`))
			session.Kill()
		})
	})
	...
	var _ = Describe("Restore cluster namespace via PITR config", func() {
		It("Should successfully restore namespace via PITR Config", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			session.Wait(2)
			Expect(session.Out).Should(gbytes.Say("The namespace test_ysql_db in cluster stunning-sole is being restored via PITR Configuration."))
			session.Kill()
		})
	})

	var _ = Describe("Describe Cluster PITR Config", func() {
		It("Should successfully describe PITR Configs for the namespace in the cluster", func() {
			err := loadJson("./test/fixtures/list-cluster-pitr-configs.json", &responseListPITRConfig)
			Expect(err).ToNot(HaveOccurred())
			server.AppendHandlers(
				ghttp.CombineHandlers(
			...
			getErr := loadJson("./test/fixtures/get-cluster-pitr-config.json", &responseGetPITRConfig)
			...
			cmd := exec.Command(compiledCLIPath, "cluster", "pitr-config", "describe", "--cluster-name", "stunning-sole", "--namespace-name", "test_ysql_db")
			session, err := gexec.Start(cmd, GinkgoWriter, GinkgoWriter)
			Expect(err).NotTo(HaveOccurred())
			session.Wait(2)
			Expect(session.Out).Should(gbytes.Say(`Namespace      Table Type   Retention Period in Days   Backup Interval in Seconds   State     Earliest Recovery Time in Millis
test_ysql_db   YSQL         5                          86400                        QUEUED    654321`))
		...
		It("should fail if no PITR Configs found for the cluster", func() {
			...
			cmd := exec.Command(compiledCLIPath, "cluster", "pitr-config", "describe", "--cluster-name", "stunning-sole", "--namespace-name", "different-db")
			session, err := gexec.Start(cmd, GinkgoWriter, GinkgoWriter)
			Expect(err).NotTo(HaveOccurred())
			session.Wait(2)
			Expect(session.Err).Should(gbytes.Say("No PITR Configs found for namespace different-db in cluster stunning-sole.\n"))
			session.Kill()
		})
	})

	var _ = Describe("Delete Cluster PITR Config", func() {
		It("Should successfully delete PITR Configs for the namespace in the cluster", func() {
			err := loadJson("./test/fixtures/list-cluster-pitr-configs.json", &responseListPITRConfig)
			Expect(err).ToNot(HaveOccurred())
			server.AppendHandlers(
				ghttp.CombineHandlers(
			...
			cmd := exec.Command(compiledCLIPath, "cluster", "pitr-config", "delete", "--cluster-name", "stunning-sole", "--namespace-name", "test_ysql_db", "-f")
			session, err := gexec.Start(cmd, GinkgoWriter, GinkgoWriter)
			Expect(err).NotTo(HaveOccurred())
			session.Wait(2)
			Expect(session.Out).Should(gbytes.Say("The PITR Configuration for namespace test_ysql_db in cluster stunning-sole is being removed."))
			session.Kill()
		})
		...
		It("should fail if no PITR Configs found for the cluster", func() {
			...
			cmd := exec.Command(compiledCLIPath, "cluster", "pitr-config", "delete", "--cluster-name", "stunning-sole", "--namespace-name", "different-db", "-f")
			session, err := gexec.Start(cmd, GinkgoWriter, GinkgoWriter)
			Expect(err).NotTo(HaveOccurred())
			session.Wait(2)
			Expect(session.Err).Should(gbytes.Say("No PITR Configs found for namespace different-db in cluster stunning-sole.\n"))
